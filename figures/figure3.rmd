...
Script to regenerate figure 3, the MOI estimation boxplot of scenario 2 (intermediate transmission), after merging identical strains for each host.
...
box<-ggplot(MOI_combine,aes(x=as.factor(true_MOI),y=error,color=method))+theme_bw()+xlab(paste("True MOI","(number of hosts)",sep="\n"))+ylab("Error in MOI")+theme_clean()+
  ...
  scale_color_manual(values=color)+scale_y_continuous(limits=c(-9.2,10.2),breaks=seq(-9,10,1))+
